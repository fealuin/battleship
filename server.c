...
{
  int jugador;
  int barcos[TAMANO_TABLERO][TAMANO_TABLERO];
  int disparos[TAMANO_TABLERO][TAMANO_TABLERO];
} tablero;

int numeroJugadores;

void limpiaTablero(tablero t){
    int i,j;
    for(i=0;i<TAMANO_TABLERO;i++){
        for (int j = 0; i < TAMANO_TABLERO; j++)
        {
            t.barcos[i][j]=0;
            t.disparos[i][j]=0;
        }
...
void *connection_handler(void *socket_desc)
{
    ...
    message = "Bienvenido al juego battleship\n";
